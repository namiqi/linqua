...
export function ReviewClient({ lesson, words }: ReviewClientProps) {
  ...
  const currentIndex = toReview.findIndex((w) => !reviewed.has(w.lemma));
  const current = currentIndex >= 0 ? toReview[currentIndex] : null;
  ...
  const mark = useCallback(
    (status: "known" | "learning") => {
      ...
      startTransition(async () => {
        await markWordAction(lesson.id, current.lemma, status, translation);
        setReviewed((prev) => new Set(prev).add(current.lemma));
      });
    },
    [current, lesson.id, translation]
  );
        ...
        {done ? (
          ...
          <div className="mt-8 rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
            <p className="text-sm text-slate-500">
              Word {reviewedCount + 1} of {total}
              {current.occurrence_count > 1 && ` · appears ${current.occurrence_count}× in transcript`}
            </p>
          ...
          <p className="mt-8 text-center text-slate-500">No words to review in this lesson.</p>
        )}
      </main>
    </div>
  );
